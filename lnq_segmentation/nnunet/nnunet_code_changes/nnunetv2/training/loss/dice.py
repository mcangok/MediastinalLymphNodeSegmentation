...
class SoftDiceLoss(nn.Module):
    def __init__(self, apply_nonlin: Callable = None, batch_dice: bool = False, do_bg: bool = True, smooth: float = 1.,
                 ddp: bool = True, clip_tp: float = None, pseudo_labeling: bool = False):
        """
        """
...
class MemoryEfficientSoftDiceLoss(nn.Module):
    def __init__(self, apply_nonlin: Callable = None, batch_dice: bool = False, do_bg: bool = True, smooth: float = 1.,
                 ddp: bool = True, pseudo_labeling: bool = False, T1=0, T2=0):
        """
    ...
    def forward(self, x, y, loss_mask=None, epoch=0):
        shp_x, shp_y = x.shape, y.shape
        pseudo_mask = torch.where(loss_mask == False, True, False)
        max_input = torch.max(torch.clone(x).detach(), dim=1, keepdim=True).values
        ...
        with torch.no_grad():
            ...
            if self.pseudo_labeling:
                sum_gt_pseudo = (y_onehot * confidence_mask).sum(axes)
        ...
        if self.pseudo_labeling:
            intersect_pseudo = (x * y_onehot * confidence_mask).sum(axes)
            sum_pred_pseudo = (x * confidence_mask).sum(axes)
        ...
        if self.batch_dice:
            ...
            if self.pseudo_labeling:
                intersect_pseudo = intersect_pseudo.sum(0)
                sum_pred_pseudo = sum_pred_pseudo.sum(0)
                sum_gt_pseudo = sum_gt_pseudo.sum(0)

        #print('dc loss forward')

        dc = (2 * intersect + self.smooth) / (torch.clip(sum_gt + sum_pred + self.smooth, 1e-8))
        if self.pseudo_labeling:
            dc_pseudo = (2 * intersect_pseudo + self.smooth) / (torch.clip(sum_gt_pseudo + sum_pred_pseudo + self.smooth, 1e-8))
        ...
        if self.pseudo_labeling:
            ...
            if epoch < self.T1:
                ...
                alpha = 1 - (((epoch - self.T2) / (100-self.T2))*1)
                alpha = 1
            ...
            # print(dc.mean())
            # print('dc_pseudo')
            # print(dc_pseudo.mean())

            dc = (labeled_voxel_weighting * dc.mean()) + (alpha * pseudo_voxel_weighting * dc_pseudo.mean())
        else:
            dc = dc.mean()
