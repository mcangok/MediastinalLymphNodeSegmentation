...
class DeepSupervisionWrapper(nn.Module):
    ...
    def forward(self, *args):
        for i in args[:-2]:
            ...
            # args[-1] is current_epoch
        current_epoch = args[-2]
        train = args[-1]
        ...
        # really necessary
        l = weights[0] * self.loss(*[j[0] for j in args[:-2]], current_epoch, train)
        for i, inputs in enumerate(zip(*args[:-2])):
            if i == 0:
                continue
            l += weights[i] * self.loss(*inputs, current_epoch, train)
        return l