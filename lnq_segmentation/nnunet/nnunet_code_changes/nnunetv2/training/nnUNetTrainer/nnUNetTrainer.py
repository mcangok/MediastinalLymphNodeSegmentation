...
class nnUNetTrainer(object):
    def __init__(self, plans: dict, configuration: str, fold: int, dataset_json: dict, unpack_dataset: bool = True,
        ...
        ### Some hyperparameters for you to fiddle
        self.pseudo_labeling = False
        self.T1 = 20 #1000
        self.T2 = 50 #1500

        # challenge submission hyperparameters
    ...
    def _build_loss(self):
        if self.label_manager.has_regions:
            loss = DC_and_BCE_loss({},
                                   ...
                                   dice_class=MemoryEfficientSoftDiceLoss, pseudo_labeling=self.pseudo_labeling, T1=self.T1, T2=self.T2)
        else:
            loss = DC_and_CE_loss({'batch_dice': self.configuration_manager.batch_dice,
                                   'smooth': 1e-5, 'do_bg': False, 'ddp': self.is_ddp}, {}, weight_ce=1, weight_dice=1,
                                  ignore_label=self.label_manager.ignore_label, dice_class=MemoryEfficientSoftDiceLoss, pseudo_labeling=self.pseudo_labeling, T1=self.T1, T2=self.T2)
    ...
    def train_step(self, batch: dict) -> dict:
        ...
        with autocast(self.device.type, enabled=True) if self.device.type == 'cuda' else dummy_context():
            ...
            # del data
            l = self.loss(output, target, self.current_epoch, True)
    ...
    def validation_step(self, batch: dict) -> dict:
        ...
        with autocast(self.device.type, enabled=True) if self.device.type == 'cuda' else dummy_context():
            output = self.network(data)
            del data
            l = self.loss(output, target, self.current_epoch, False)
